...
def compute_logit_diagnostics(
    ...
    *,
    ...
    """
    Compute lightweight diagnostics for vocabulary-logit growth.

    To avoid a large extra memory/time cost for Gemma's large vocabulary, the
    metrics are computed on an evenly spaced subset of token positions. The
    logits tensor itself is already produced by the CausalLM forward pass.
    """
    with torch.no_grad():
        z = logits.detach()
        if z.ndim != 3:
            raise ValueError(f"Expected logits with shape [B, T, V], got {tuple(z.shape)}")

        n_pos = z.shape[0] * z.shape[1]
        vocab = z.shape[-1]
        z = z.reshape(n_pos, vocab)

        if max_positions is not None and max_positions > 0 and n_pos > max_positions:
            idx = torch.linspace(
                0,
                n_pos - 1,
                steps=max_positions,
        ...
        log_probs = z - logsumexp.unsqueeze(-1)
        probs = torch.exp(log_probs)
        entropy = -(probs * log_probs).sum(dim=-1)
        prob_max = probs.max(dim=-1).values

        return {
            "logits/rms": z.square().mean().sqrt(),
            "logits/centered_rms": z_centered.square().mean().sqrt(),
            "logits/max_abs": z.abs().max(),
            "logits/centered_max_abs": z_centered.abs().max(),
            "logits/logsumexp_mean": logsumexp.mean(),
            "logits/logsumexp_max": logsumexp.max(),
            "logits/prob_entropy_mean": entropy.mean(),
            "logits/prob_max_mean": prob_max.mean(),
        }


def reduce_logit_metrics(metrics: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    """Average mean-like metrics across ranks and take max for max-like metrics."""
    ...
    for key, value in metrics.items():
        ...
        if key.endswith("/max_abs") or key.endswith("/centered_max_abs") or key.endswith("/logsumexp_max"):
...
def log_metrics_to_tensorboard(
    ...
    *,
    ...
    for key, value in metrics.items():
        writer.add_scalar(f"{prefix}/{key}", float(value.detach().item()), step)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device_batch_size", type=int, default=18)
    parser.add_argument("--hidden_size", type=int, default=1152)
    parser.add_argument("--num_hidden_layers", type=int, default=26)
    ...
    parser.add_argument("--val_tokens", type=int, default=10_469_376)
    parser.add_argument("--val_loss_every", type=int, default=500)
    parser.add_argument("--val_bin_pattern", type=str, default="*_val_*.bin")
    parser.add_argument("--final_logit_softcapping", type=float, default=None)
    parser.add_argument("--logit_metrics_train_every", type=int, default=500)
    parser.add_argument("--logit_metrics_num_positions", type=int, default=64)
    args = parser.parse_args()

    # Default SwiGLU MLP optimizer hyperparameters to the generic matrix/Muon values.
    ...
    if ddp_is_main():
        ...
        print(f"final_logit_softcapping: {args.final_logit_softcapping}", flush=True)
        print(f"logit_metrics_num_positions: {args.logit_metrics_num_positions}", flush=True)
    ...
    # HF Gemma 3 stores Linear weights as [out_features, in_features].
    ...
    for module_name, module in raw_model.named_modules():
        if all(hasattr(module, attr) for attr in ("gate_proj", "up_proj", "down_proj")):
            ...
            if ddp_is_main() and module_name.endswith("mlp"):
                ...
                break
    assert found_swiglu_mlp, "Expected at least one Gemma 3 SwiGLU MLP with gate_proj/up_proj/down_proj."
    ...
    # Do not pass SwiGLU MLP params as stale metadata to the matrix/Muon optimizer
    # when --mlp_*_optimizer is row/hybrid/adamw. The routing file will handle
    # gate_proj/up_proj as mlp_gate_up and down_proj as mlp_down.
    ...
    optimizer = build_transformer_mixed_optimizer(
        ...
        lr_mlp_gate_up=args.lr_mlp_gate_up,
        lr_mlp_down=args.lr_mlp_down,
        wd_other=args.weight_decay,
        wd_matrix=args.wd_muon,
        wd_embed=args.wd_embed,
        wd_lm_head=args.wd_lm_head,
        wd_mlp_gate_up=args.wd_mlp_gate_up,
        wd_mlp_down=args.wd_mlp_down,
        beta_matrix=args.beta_matrix,
        beta_embed=args.beta_embed,
        beta_lm_head=args.beta_lm_head,
        beta_mlp_gate_up=args.beta_mlp_gate_up,
        beta_mlp_down=args.beta_mlp_down,
        alpha=args.alpha,
        eps=args.eps,
    ...
    for step in range(args.train_steps + 1):
        ...
        if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
            ...
            for _ in range(val_steps):
                with torch.no_grad():
                    ...
                    batch_logit_metrics = compute_logit_diagnostics(
                        outputs.logits,
                        max_positions=args.logit_metrics_num_positions,
                    )
                    if val_logit_metric_sums is None:
                        val_logit_metric_sums = {
                            k: torch.zeros_like(v) for k, v in batch_logit_metrics.items()
                        }
                    for k, v in batch_logit_metrics.items():
                        val_logit_metric_sums[k].add_(v.detach())
            ...
            val_logit_metrics = {
                k: v / val_steps for k, v in val_logit_metric_sums.items()
            }
            val_logit_metrics = reduce_logit_metrics(val_logit_metrics)
            ...
            if ddp_is_main():
                print(
                    " | ".join(
                        [f"val_{k.replace('/', '_')}:{float(v.item()):.6f}" for k, v in val_logit_metrics.items()]
                    ),
                    flush=True,
                )
            ...
            if args.tensorboard and ddp_is_main():
                ...
                log_metrics_to_tensorboard(writer, val_logit_metrics, prefix="val", step=step)
        ...
        train_logit_metrics = None
        if args.logit_metrics_train_every > 0 and step % args.logit_metrics_train_every == 0:
            train_logit_metrics = compute_logit_diagnostics(
                outputs.logits,
                max_positions=args.logit_metrics_num_positions,
            )
            train_logit_metrics = reduce_logit_metrics(train_logit_metrics)
        ...
        if train_logit_metrics is not None and ddp_is_main():
            print(
                " | ".join(
                    [f"train_{k.replace('/', '_')}:{float(v.item()):.6f}" for k, v in train_logit_metrics.items()]
                ),
                flush=True,
            )
        ...
        if args.tensorboard and ddp_is_main():
            ...
            if train_logit_metrics is not None:
                log_metrics_to_tensorboard(writer, train_logit_metrics, prefix="train", step=step)
